"""
CUDA Platform - Qwen3 End-to-End Training Test

Supports single GPU, DDP, and FSDP with NCCL or FlagCX backends.

Usage:
    # Single GPU
    python tests/cuda/test_qwen3_train.py

    # DDP with NCCL
    torchrun --nproc_per_node=2 tests/cuda/test_qwen3_train.py --parallel ddp --comm nccl

    # DDP with FlagCX
    torchrun --nproc_per_node=2 tests/cuda/test_qwen3_train.py --parallel ddp --comm flagcx

    # FSDP with NCCL
    torchrun --nproc_per_node=2 tests/cuda/test_qwen3_train.py --parallel fsdp --comm nccl

    # FSDP with FlagCX
    torchrun --nproc_per_node=2 tests/cuda/test_qwen3_train.py --parallel fsdp --comm flagcx
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="CUDA Qwen3 Training Test")
...
def setup(args):
    ...
    if args.parallel == "none":
        return f"cuda:{local_rank}", local_rank, 1, 0
    ...
    rank = dist.get_rank()
    return f"cuda:{local_rank}", local_rank, world_size, rank
...
def wrap_ddp(model, args, local_rank, rank):
    ...
    print_rank0("    DDP: CUDA mode", rank)
    return model
...
def wrap_fsdp(model, args, device, rank):
    ...
    policy = functools.partial(transformer_auto_wrap_policy, transformer_layer_cls={Qwen3DecoderLayer})
    model = FSDP(
        model,
    ...
    model.train()
    print_rank0("    FSDP: FULL_SHARD (CUDA)", rank)
    return model
...
def print_summary(args, step_times, total_loss, total_tokens, world_size, rank):
    print_rank0("\n" + "=" * 60, rank)
    print_rank0(f"Training Summary (device=cuda, parallel={args.parallel}, comm={args.comm})", rank)
...
def main():
    ...
    print_rank0("=" * 60, rank)
    print_rank0("CUDA Qwen3 Training Test", rank)
    print_rank0("=" * 60, rank)
    print_rank0(f"device=cuda  parallel={args.parallel}  comm={args.comm}", rank)
