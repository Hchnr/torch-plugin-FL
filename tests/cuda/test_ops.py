"""
CUDA Platform - Basic Operations and Tensor Tests

Usage:
    pytest tests/cuda/test_ops.py -v
"""
...
def pytest_configure(config):
    if not torch.cuda.is_available():
        pytest.exit("CUDA is not available on this machine.")
...
class TestActivations:
    ...
    def test_softmax(self):
        ...
        assert torch.allclose(sm.sum(dim=-1), torch.ones(64, device=DEVICE), atol=1e-5)
